...
//		=> 명시적 객체 형변환을 사용하여 Object 타입의 객체를 JButton 객체로 변환받아 저장
//		Object eventSource = e.getSource();
		JButton eventButton = (JButton)e.getSource();
		
//		이벤트가 발생된 JButton 컴퍼넌트를 비교하여 명령을 선택 실행하는 메소드 작성
		if(eventButton == b_C) {//이벤트가 발생된 Button 컴퍼넌트가 [c]인 경우 
//			연산식을 저장하기 위한 필드 초기화로 "" 넣고
			operation = "";
//			JLabel 컴퍼넌트를 0으로 초기화
			label.setText("0");
			
		} else if(eventButton == b_equals){// 이벤트가 발생된 Button 컴퍼넌트가 [=]인 경우 
//			연산식에서 검색하기 위한 연산자가 저장된 배열을 선언하고
			String[] operatorArray = {"*","/","+","-"};
			
//			연산식에서 연산자를 검색하기 위한 반복문
			int index = -1; // 연산자의 위치값(첨자)을 저장하기 위한 변수 index 생성
...
//				label.setText(String.valueOf(result)); 위에꺼랑 둘중하나 쓰면 됨
				
//				operation = ""; 이렇게 써도 되고 밑에처럼 써도 됨, 밑에 처럼 쓰면 사칙연산을 한큐에 여러번 가능
...
//				=> 메세지 다이얼로그를 출력하는 메소드 
				JOptionPane.showMessageDialog(this, "입력한 연산식이 형식에 어긋남");
				label.setText("0");
				operation = "";
...
//			operation 필드에 저장된 문자열을 사용하여 JLabel 컴퍼넌트의 문자열을 변경한다
