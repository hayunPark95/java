...
// 이벤트 처리 프로그램 작성 방법
// 1. 컴퍼넌트와 컨테이너 관련 클래스를 사용하여 디자인 클래스 작성 - UI구현
...
// 실행되도록 설정
// => 형식) Component.addXXXListener(Listener l) : 컴퍼넌트의 이벤트를 처리하기 위한 이벤트 처리 객체 등록
...
public class EventHandleApp extends Frame {
   ...
   public EventHandleApp(String title) {
      ...
      exit.addActionListener(new EventHanlde());
...
class EventHanlde implements ActionListener{
   @Override
